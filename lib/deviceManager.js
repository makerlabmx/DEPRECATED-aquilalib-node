	...
	self.devicesDB.find({address: device.address}, function(err, docs)
		{
			...
			{
				console.log(docs);
				// check if default name has changed, if it has, dont use saved name.
				if(docs[0].defaultName != device._defaultName) self.setDeviceName(device.address, device._defaultName, device._defaultName);
				else
				{
			...
			else
			{
				self.devicesDB.insert({ address: device.address, 
										name: device.name,
										defaultName: device._defaultName });
			}
			if(!self.deviceAlreadyAdded(device.address))
			{
...
DeviceManager.prototype.setDeviceName = function(address, name, defaultName)
{
	var self = this;
	var query = {$set: {address: address, name: name}};
	if(defaultName) query = {$set: {address: address, name: name, defaultName: defaultName}};
	self.devicesDB.update({address: address}, query, {}, function(err, replaced)
		{
			if(err) { console.log(err); return; }
